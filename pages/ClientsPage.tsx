...
const ClientsPage: React.FC = () => {
  ...
  const filteredClients = useMemo(() => {
    return clients.filter(client => {
        if (client.role === 'admin') return false;
              ...
              <table className="w-full text-sm text-right">
                <thead className="bg-light-bg/50 text-slate-400">
                  <tr>
                    ...
                    <th scope="col" className="p-4 font-semibold">حالة الإعداد</th>
                ...
                <tbody className="divide-y divide-slate-100/10">
                  {filteredClients.map((client) => (
                    <tr key={client.phone} className="hover:bg-light-bg/30 transition-colors cursor-pointer" onClick={() => window.location.href = `/dashboard/clients/${client.phone}`}>
                      ...
                      <td className="p-4 whitespace-nowrap">
                        {client.has_completed_onboarding ? (
                            <Badge color="green">مكتمل</Badge>
                        ) : (
                            <Badge color="yellow">بحاجة لإكمال</Badge>
                        )}
                      </td>
