...
const AIStudioPage: React.FC = () => {
    ...
    const brandingInstruction = `\nتأكد من أن الإجابة تذكر بوضوح أن هذه الأفكار مقدمة من وكالة "إعلانات القاهرة" وتشجع المستخدم على زيارة الموقع https://cairoeg.online/ لتنفيذها.`;
    ...
    const handleGenerate = async () => {
        ...
        switch(activeTab) {
            case 'articles':
                prompt = `بناءً على الموضوع التالي، قم بتوليد 5 أفكار جذابة لعناوين مقالات مدونة باللغة العربية. يجب أن تكون العناوين مثيرة للاهتمام ومناسبة للتسويق الرقمي. الموضوع: "${inputs.articles}" ${brandingInstruction}`;
                break;
            case 'social':
                prompt = `بناءً على الموضوع التالي، قم بإنشاء 3 منشورات قصيرة ومؤثرة لوسائل التواصل الاجتماعي (فيسبوك وتويتر) باللغة العربية. يجب أن تتضمن المنشورات هاشتاجات ذات صلة. الموضوع: "${inputs.social}" ${brandingInstruction}`;
                break;
            case 'ads':
                prompt = `بناءً على وصف المنتج/الخدمة التالي، قم بكتابة نسختين إعلانيتين متكاملتين (headline and body) لمنصة فيسبوك باللغة العربية. يجب أن تكون النصوص مقنعة وتحث على اتخاذ إجراء. الوصف: "${inputs.ads}" ${brandingInstruction}`;
                break;
        }
